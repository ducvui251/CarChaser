...
namespace CrashyChasy
{
    public class CharacterScroller : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            selectionIndex.text = CharacterManager.Instance.CurrentCharacterIndex + "/" + CharacterManager.Instance.characters.Length;
            ...
            for (int i = 0; i < CharacterManager.Instance.characters.Length; i++)
            {
                ...
                //Set layer to each of the parts of model for the camera culling, Car -> model -> parts
                Transform modelTransform = character.transform.GetChild(0);
                int partsCount = modelTransform.childCount;

                for (int j = 0; j < partsCount; j ++)
                {
                    modelTransform.GetChild(j).gameObject.layer = LayerMask.NameToLayer("CharacterSelectionUI");
                }

               
            }

            //Debug.Log("List character: " + listCharacter);
        ...
        void Update()
        {
            ...
            else if (Input.GetMouseButton(0))   // touch stays
            {
                ...
                if (deltaX >= swipeThresholdX)
                {
                    ...
                    dir = (startPos.x - endPos.x < 0) ? Vector3.right : Vector3.left;
            ...
            if (Input.GetMouseButtonUp(0))
            {
                if (hasMoved)
                {
                    ...
                    if (lastCurrentCharacter != currentCharacter)
                    {
                        currentCharacterIndex = currentCharacterIndex + (dir == Vector3.left ? 1 : -1);
                    }
                }
            }
            ...
            selectionIndex.text = (currentCharacterIndex + 1) + "/" + CharacterManager.Instance.characters.Length;
